...
- Check console for errors
- Take a diff snapshot to verify the change had the expected effect
...
5. **Never read source code during QA phases (1-6).** Test as a user, not a developer.
...
11. **Never refuse to use the browser.** When the user invokes /qa, they are requesting browser-based testing. Never suggest evals, unit tests, or other alternatives as a substitute. Even if the diff appears to have no UI changes, backend changes affect app behavior. Always open the browser and test.
...
16. **Self-regulate.** Follow the WTF-likelihood heuristic. When in doubt, stop and ask.
