...
Look for a design doc in the project directory or workspace. Check for files matching patterns like `*design*.md`, `*architecture*.md`, `*tech-design*.md` in the project root, `docs/`, or `doc/` directories.

If a design doc exists, read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design -- check the prior version for context on what changed and why.

### Prerequisite Skill Offer

When no design doc is found, mention it to the user:

> "No design doc found for this project/branch. A structured problem statement, premise challenge, and explored alternatives would give this review much sharper input to work with. Consider writing one before or after this review."

Then proceed with the standard review.
...
**STOP.** For each issue found in this section, present it individually. One issue at a time. Present options, state your recommendation, explain WHY. Do NOT batch multiple issues into one question. Only proceed to the next section after ALL issues in this section are resolved.
...
For LLM/prompt changes: check the "Prompt/LLM changes" file patterns listed in CLAUDE.md. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against. Then confirm the eval scope with the user.
...
This file helps QA testers know **what to test and where** -- not implementation details.
...
### TODOS.md updates
After all review sections are complete, present each potential TODO individually. Never batch TODOs. Never silently skip this step.
