...
> This skill requires a browser automation tool to take screenshots, extract rendered styles, and
> interact with pages. Use Playwright MCP, Puppeteer, or any equivalent headless browser tool
> available in your environment. The commands below use pseudocode (`browser.screenshot`,
> `browser.evaluate`, `browser.navigate`, etc.). Map these to whichever browser tool you have.
...
If user picks C, tell them: "If you change your mind later, re-run the skill and we'll set it up." Continue without tests.
...
Use browser automation to evaluate these in the page:
...
3. Take responsive screenshots (mobile, tablet, desktop)
...
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. Ask the user how to proceed (import cookies, provide credentials, etc.).
...
- Ellipsis character used, not three dots
...
For each flow, take snapshots before and after interactions, then use diff to see what changed.
...
1. Take a screenshot of the fixed state
...
- **reverted**: regression detected, run `git revert HEAD`, mark finding as "deferred"
...
the exact bug condition, run it, commit if passes or defer if fails.
...
- Deferred findings
- Design score delta: baseline to final
- AI slop score delta: baseline to final

**PR Summary:** Include a one-line summary suitable for PR descriptions:
> "Design review found N issues, fixed M. Design score X to Y, AI slop score X to Y."
...
11. **Clean working tree required.** If dirty, ask the user to commit/stash/abort before proceeding.
