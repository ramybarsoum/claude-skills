...
**Backlog Health (if TODOS.md exists):** Read `TODOS.md` (fetched in Step 1). Compute:
...
**If only one contributor (solo repo):** Skip the team breakdown and proceed as a personal retro.
...
**AI collaboration note:** If many commits have `Co-Authored-By` AI trailers (e.g., Claude, Copilot), note the AI-assisted commit percentage as a team metric. Frame it neutrally: "N% of commits were AI-assisted."
...
1. Compute metrics for the current window (default 7d) using the midnight-aligned start date (same logic as the main retro)
